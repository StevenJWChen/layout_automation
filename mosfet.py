...
class MOSFET:
    ...
    def generate(self) -> Cell:
        ...
        # Get design rules
        poly_width = self.length
        poly_extension = nm(130)  # Extension beyond active area
        diff_contact_spacing = nm(55)  # Space from gate to contact
        contact_size = nm(170)

        # Calculate dimensions
        # For each finger: gate + spacing on each side
        finger_pitch = poly_width + 2 * diff_contact_spacing + contact_size
        ...
        for i in range(self.fingers):
            drain_x = contact_size + i * finger_pitch + poly_width + diff_contact_spacing + contact_size // 2
            ...
            if i < self.fingers - 1:
                next_source_x = drain_x + contact_size // 2 + diff_contact_spacing + poly_width + diff_contact_spacing + contact_size // 2
                source_contact = Contact(
